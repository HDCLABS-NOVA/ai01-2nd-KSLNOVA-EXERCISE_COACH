...
│       └── styles/main.css   # 스타일
└── backend/
    ├── exercise_server.py    # FastAPI REST (OpenAI/YouTube/분석)
    ├── db.py                 # SQLite 기록/세션 관리 유틸
    ├── exercise_records.db   # 로컬 SQLite DB (실행 중 생성/사용)
    └── exercise_data/        # 로그·캡처 저장 폴더(필요 시 생성)
```
...
- [FR-07] API 키·엔드포인트는 환경변수(백엔드 export)로 주입하며, 필요 시 env.js 생성 옵션을 제공한다.
- [FR-08] 운동 종료 시 카메라 스트림·카운트/로그를 정지·리셋할 수 있다.

## 2. 비기능 요구사항
- [NFR-01] 로컬 데모 기준 저지연(수백 ms~1s) 피드백을 목표로 한다.
- [NFR-02] API 키 누락/쿼터 초과 시 기본 안내·대체 영상으로 폴백한다.
- [NFR-03] 키 노출 방지를 위해 env를 사용하고, env.js는 Git에 포함하지 않는다(옵션).
- [NFR-04] 외부 네트워크(OpenAI/YouTube) 실패 시에도 카메라 스트림·기본 영상은 동작해야 한다.
...
2. **Frontend(UI)**: React(JSX/CDN) 단일 페이지, FastAPI에서 정적 서빙(`http://localhost:8003/`), 로컬 Mediapipe로 관절/rep 계산·스켈레톤 표시, 최하단 캡처를 REST로 전송, 튜토리얼/채팅/TTS UI  
3. **FastAPI REST 서버(`exercise_server.py`)**: `/analyze-image`(캡처→OpenAI 피드백), `/chat`, `/search-youtube`, `/save-log`, `/save-session`, 정적 파일(index.html 등) 서빙  
...
7. **데이터/로그**: 프론트 메모리 세션(역사/rep/feedback) + 백엔드 SQLite(`exercise_records.db`), 필요 시 `exercise_data/`에 로그·영상 저장
...
**CV / 분석**
<p align="center">
  <img src="https://img.shields.io/badge/Mediapipe%20Pose-00C7B7?style=for-the-badge&logo=google&logoColor=white"/>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Static%20Hosting/Serve-0F172A?style=for-the-badge&logo=html5&logoColor=white"/>
...
- 외부 키: `OPENAI_API_KEY`, `YOUTUBE_API_KEY` (백엔드 환경변수), 공개 저장소에 포함 금지
- 로컬 파일: `exercise_records.db`(SQLite), `exercise_data/`(로그·영상 저장), 필요 시 `env.js`(런타임 키/엔드포인트)

## 2. 연동 방식
- REST `/analyze-image`: 프론트 최하단 캡처 → 서버 → OpenAI → 피드백 JSON 반환
- REST `/chat`, `/search-youtube`, `/save-log`, `/save-session`: 채팅·추천·로그/영상 저장 처리
- 정적 서빙: FastAPI에서 `frontend/index.html` 등 UI 파일 제공
...
http://localhost:8003/
```
※ 프론트 정적 서버를 별도로 띄우지 않는 현재 구조 기준. env.js가 필요한 경우 HOWTORUN.md의 정적 프론트 실행 절차를 참고.
...
## 1. 아키텍처 개요
- 기본은 로컬 FastAPI(정적 + REST) 데모로 동작하며, 필요 시 클라우드에 배포:
  - 프론트: FastAPI에서 정적 서빙 또는 별도 정적 호스팅(S3/CloudFront, Vercel 등)
  - 백엔드: FastAPI + Uvicorn (예: EC2/Cloud Run), REST 엔드포인트(`/analyze-image`, `/chat`, `/search-youtube`, `/save-log`, `/save-session`)
  - 데이터: SQLite(또는 관리형 DB) + 스토리지(`exercise_data/` 대체), 로그/캡처 보관
  - 보안: 키는 서버 환경변수, 프론트 배포 시 env.js는 런타임 주입(비공개)

## 2. 설계 이미지
```
[Browser]
  - frontend/index.html + src/* (React + Mediapipe, FastAPI 정적 서빙)
  - env 주입(필요 시 env.js)
        |
        v
